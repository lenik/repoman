...
DRM_HEALTH_FILE=""
DRM_PREFERRED_FILE=""
DRM_GETBAR="${DRM_GETBAR:-getbar}"
DRM_GETBAR_OPTS="${DRM_GETBAR_OPTS:--c -d2 -p1 -w3 -s30m -i.1 -q}"
# Reference download size for scoring: effective B/s = ref / (offset + ref/bps).
DRM_BWTEST_REF_BYTES="${DRM_BWTEST_REF_BYTES:-3000000}"
...
init_drm() {
    ...
    DRM_MIRROR_FILE="$DRM_STATE_DIR/mirrors"
    DRM_DEFAULT_FILE="$DRM_STATE_DIR/default"
    DRM_HEALTH_FILE="$DRM_STATE_DIR/health"
    DRM_PREFERRED_FILE="$DRM_STATE_DIR/preferred"
    mkdir -p "$DRM_STATE_DIR"
    vlog2 "state directory: $DRM_STATE_DIR"
}

_drm_ensure_state_paths() {
    [[ -n "${DRM_MIRROR_FILE:-}" ]] || \
        die "$(_ 'internal error: call init_drm before mirror commands')"
}

health_load() {
    if [[ -z "${DRM_HEALTH_LOADED:-}" ]]; then
        declare -gA DRM_HEALTH=()
        DRM_HEALTH_LOADED=1
    fi
    if [[ -f "$DRM_HEALTH_FILE" ]]; then
        local alias status
        while IFS=$'\t' read -r alias status || [[ -n "$alias" ]]; do
            [[ -n "$alias" && -n "$status" ]] || continue
            DRM_HEALTH[$alias]="$status"
        done <"$DRM_HEALTH_FILE"
    fi
}
...
health_save() {
    ...
    _drm_ensure_state_paths
    : >"$DRM_HEALTH_FILE"
    for alias in "${!DRM_HEALTH[@]}"; do
        printf '%s\t%s\n' "$alias" "${DRM_HEALTH[$alias]}" >>"$DRM_HEALTH_FILE"
    done
}
...
preferred_get() {
    if [[ -f "$DRM_PREFERRED_FILE" ]]; then
        tr -d '[:space:]' <"$DRM_PREFERRED_FILE"
    fi
}

preferred_set() {
    _drm_ensure_state_paths
    printf '%s\n' "$1" >"$DRM_PREFERRED_FILE"
}

preferred_clear() {
    _drm_ensure_state_paths
    rm -f "$DRM_PREFERRED_FILE"
}
...
mirror_list_mark() {
    ...
    if [[ -f "$DRM_DEFAULT_FILE" ]]; then
        default="$(tr -d '[:space:]' <"$DRM_DEFAULT_FILE")"
    fi
...
mirror_remove() {
    ...
    if [[ -f "$DRM_DEFAULT_FILE" ]] && [[ "$(cat "$DRM_DEFAULT_FILE")" == "$alias" ]]; then
        rm -f "$DRM_DEFAULT_FILE"
    fi

    health_load
    unset 'DRM_HEALTH[$alias]'
    health_save
    if [[ -f "$DRM_PREFERRED_FILE" ]] && [[ "$(preferred_get)" == "$alias" ]]; then
        preferred_clear
    fi
}

load_mirrors() {
    if [[ "${DRM_MIRRORS_LOADED:-0}" -eq 1 ]]; then
        return 0
    fi
    ...
    done <"$DRM_MIRROR_FILE"
    DRM_MIRRORS_LOADED=1
    vlog2 "loaded ${#MIRROR_ALIASES[@]} mirrors from $DRM_MIRROR_FILE"
}

invalidate_mirrors_cache() {
    DRM_MIRRORS_LOADED=0
}

save_mirrors() {
    local i
    _drm_ensure_state_paths
    mkdir -p "$(dirname "$DRM_MIRROR_FILE")"
    : >"$DRM_MIRROR_FILE"
    for i in "${!MIRROR_ALIASES[@]}"; do
        printf '%s\t%s\t%s\n' \
...
list_mirrors_sorted() {
    ...
    for idx in "${MIRROR_LIST_ORDER[@]}"; do
        ...
        if [[ "${DRM_LIST_PRIORITY:-0}" -eq 1 ]]; then
            printf '%s[%d] %s %s %s\n' \
...
_ping_mirror_worker() {
    ...
    if ! mapfile -t ping_lines < <(ping -c "${DRM_PING_COUNT:-3}" -W "${DRM_PING_TIMEOUT:-2}" "$host" 2>/dev/null | tail -2); then
...
_bw_score_from_output() {
    awk -v alias="$1" -v url="$2" -v ref="${DRM_BWTEST_REF_BYTES:-3000000}" '
...
_bw_mirror_worker() {
    ...
    vlog3 "getbar $DRM_GETBAR $DRM_GETBAR_OPTS $test_url"

    getbar_out="$(mktemp "${TMPDIR:-/tmp}/drm-getbar.XXXXXX")"
    if ! $DRM_GETBAR $DRM_GETBAR_OPTS "$test_url" >"$getbar_out" 2>/dev/null; then
...
run_pingtest() {
    ...
    local tmpdir i
    local -a results=()
    tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/drm-ping.XXXXXX")"
...
run_bwtest() {
    require_commands "$DRM_GETBAR" awk sort
    ((${#MIRROR_ALIASES[@]} > 0)) || die "$(_ 'no mirrors configured')"

    vlog2 "bandwidth testing ${#MIRROR_ALIASES[@]} mirrors in parallel (ref=${DRM_BWTEST_REF_BYTES} bytes)"
    local tmpdir i
    local -a results=()
    tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/drm-bw.XXXXXX")"
